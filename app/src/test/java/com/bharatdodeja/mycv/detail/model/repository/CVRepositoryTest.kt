...
class CVRepositoryTest {
    ...
    private lateinit var mTasksTestSubscriber: TestSubscriber<CVDataModel>
    ...
    fun setUp() {
        ...
        mTasksTestSubscriber = TestSubscriber()
    ...
    fun getCV_dataSourceCallIsCaptured() {
        ...
        whenDataSourceReturnsSuccess(userId, cvDataModel)

        // When getCV call is subscribed to data source
        repository.getCV(userId).subscribe(mTasksTestSubscriber)
    ...
    fun getCV_andThereIsError_repositoryReturnsError() {
        // Given remote data source returns success
        whenDataSourceReturnsError(userId, error)

        // When getCV call is subscribed to data source
        repository.getCV(userId).subscribe(mTasksTestSubscriber)

        // Then observable emits error
        mTasksTestSubscriber.assertError(error)
    }
    ...
    fun getCV_andThereIsSuccess_returnsExpectedResult() {
        ...
        mTasksTestSubscriber.assertValue(cvDataModel)
    }

    private fun whenDataSourceReturnsSuccess(userId: String, cvDataModel: CVDataModel) {
        `when`(remoteDataSource.getCV(userId)).thenReturn(Flowable.just(cvDataModel))
    }

    private fun whenDataSourceReturnsError(userId: String, error: Throwable) {
